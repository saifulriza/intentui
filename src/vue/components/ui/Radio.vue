...
const group = inject<{
  modelValue: { value?: string | number | boolean | null }
  name: { value?: string }
...
<template>
  <label class="group block" :class="disabled ? 'opacity-50' : ''">
    ...
    >
      <span
        data-slot="indicator"
        class="relative inset-ring inset-ring-input isolate flex size-4.5 shrink-0 items-center justify-center rounded-full text-bg transition before:absolute before:inset-auto before:size-2 before:shrink-0 before:rounded-full before:content-[''] hover:before:bg-muted-fg/20 sm:size-4 sm:before:size-1.7 in-disabled:bg-muted"
        :class="{
